...
export const ComparisonPanel: React.FC = () => {
  ...
  const {
    ...
    updateBalance: updateFlashBalance,
  } = useBalanceTracker(address, "flashblocks");
  const {
    balance: tradBalance,
    updateBalance: updateTradBalance,
  } = useBalanceTracker(address, "traditional");

  // Update balance when transaction tracking provides new values
  useEffect(() => {
    if (flashblocksTracker?.balanceAfter !== undefined) {
      updateFlashBalance(flashblocksTracker.balanceAfter);
    }

  }, [flashblocksTracker?.balanceAfter, updateFlashBalance]);

  useEffect(() => {
    if (traditionalTracker?.balanceAfter !== undefined) {
      updateTradBalance(traditionalTracker.balanceAfter);
    }

  }, [traditionalTracker?.balanceAfter, updateTradBalance]);

  const flashMetrics: ComparisonMetrics = {
    balance: isConnected ? flashBalance : BigInt(0),
    transactionTracker: flashblocksTracker,
  }

  const tradMetrics: ComparisonMetrics = {
    balance: isConnected ? tradBalance : BigInt(0),
      ...
      <div className="flex flex-col items-center gap-4">
        <div className="flex gap-3 w-full max-w-md">
          <Button
            variant="outline"
            size="lg"
            onClick={() => window.open(GIWA_LINKS.FAUCET, "_blank")}
          >
            Get Testnet ETH
